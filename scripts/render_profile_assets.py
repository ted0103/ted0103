...
def outlined(font: TTFont, value: str, x: float, y: float, size: float, fill: str, anchor: str = "start") -> str:
    ...
    for name in names:
        ...
        if data:
            paths.append(
                f'<path d="{data}" transform="translate({cursor:.2f} {y:.2f}) scale({scale:.6f} {-scale:.6f})" fill="{fill}"/>'
            )
...
def hero(fonts: dict[str, TTFont]) -> str:
    ...
    body = (
        ...
        + outlined(fonts["display"], "TAN TED HANG", 58, 223, 78, "#FFFFFF")
